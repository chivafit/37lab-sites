...
export default async function DemoPage({params}:PageProps){
 ...
 return <main className="medicalSite" style={themeStyle}>
  ...
  <section id="servicos" className="services reveal"><div className="sectionIntro"><div className="eyebrow">PRINCIPAIS ÁREAS DE ATUAÇÃO</div><h2>Como posso te ajudar?</h2><p>Uma jornada de cuidado pensada para prevenção, orientação e acompanhamento.</p></div><div className="cards">{p.services.map((s,i)=><article key={s.title} style={{animationDelay:`${i*70}ms`}}><span className="cardNumber">0{i+1}</span><Icon type={s.icon||'plus'}/><h3>{s.title}</h3><p>{s.description}</p><span className="cardArrow">↗</span></article>)}</div></section>
  {(p.clinic||p.address)&&<section id="local" className="location reveal"><div className="locationMedia">{p.clinicImage?<img className="clinicPhoto" src={p.clinicImage} alt={p.clinic||'Clínica'}/>:<div className="clinicPlaceholder"><span>Imagem da clínica / consultório</span></div>}</div><div className="locationCopy"><div className="eyebrow">ONDE ATENDO</div><h2>{p.clinic}</h2>{p.address&&<p className="locationLine"><Icon type="pin"/>{p.address}</p>}{p.phone&&<p className="locationLine"><Icon type="phone"/>{p.phone}</p>}<a className="mapButton" href={`https://www.google.com/maps/search/?api=1&query=${mapQuery}`} target="_blank" rel="noreferrer">Ver localização <span>↗</span></a></div><a className="mapMock" href={`https://www.google.com/maps/search/?api=1&query=${mapQuery}`} target="_blank" rel="noreferrer" aria-label="Abrir localização no Google Maps"><div className="mapGrid"></div><div className="mapPin">●</div><span>Abrir mapa ↗</span></a></section>}
  <section id="faq" className="faq reveal"><div><div className="eyebrow">DÚVIDAS FREQUENTES</div><h2>Antes da consulta</h2><p>Algumas informações podem ser confirmadas diretamente com a equipe no momento do agendamento.</p></div><div className="faqList">{faqs.map(f=><details key={f.question}><summary>{f.question}<span className="faqPlus">+</span></summary><p>{f.answer}</p></details>)}</div></section>
  <section id="contato" className="cta reveal"><div><div className="eyebrow">VAMOS CUIDAR DA SUA SAÚDE?</div><h2>Seu cuidado começa<br/>com uma conversa.</h2><p>Entre em contato para consultar disponibilidade e informações de atendimento.</p></div>{p.whatsapp&&<a href={`https://wa.me/${p.whatsapp}`} target="_blank" rel="noreferrer">Agendar pelo WhatsApp <span>↗</span></a>}</section>
  <footer><div className="footerBrand"><strong>{p.shortName}</strong><span>{p.specialty}</span></div><div className="footerNav"><a href="#sobre">Sobre</a><a href="#servicos">Atuação</a><a href="#local">Local</a><a href="#faq">Dúvidas</a></div><div className="footerSocial">{p.instagram&&<a href={p.instagram} target="_blank" rel="noreferrer" aria-label="Instagram"><Icon type="instagram"/></a>}{p.whatsapp&&<a href={`https://wa.me/${p.whatsapp}`} target="_blank" rel="noreferrer" aria-label="WhatsApp"><Icon type="phone"/></a>}</div><div className="footerBottom"><span>© {new Date().getFullYear()} {p.name}</span><span>Conceito e tecnologia <b>37LAB</b></span></div></footer>
 </main>
}